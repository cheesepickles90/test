...
public class TTFRenderer
{
	...
	public TTFRenderer(Minecraft minecraft, Object obj, int i)
	{
		this(minecraft, obj, i, 31, 127);
	}

	public TTFRenderer(Minecraft minecraft, Object obj, int i, int j, int k)
	{
		startChar = j;
		endChar = k;
		xPos = new int[k - j];
	...
	public void drawGoodStringWithOutScalingAndShit(Gui g, String s, int i, int j, int colour)
	{
		...
		Color c = new Color(red,green,blue,1F);
		i*= 2;
		j*= 2;
		j-= 5;
		GL11.glPushMatrix();
	...
	private void drawChar(Gui gui, char c, double i, double j) {
		...
		gui.drawTexturedModalRect(i, j - 2, xPos[(byte)c - startChar], yPos[(byte)c - startChar], rectangle2d.getWidth(), rectangle2d.getHeight() + metrics.getMaxDescent() + 2);
	}
}