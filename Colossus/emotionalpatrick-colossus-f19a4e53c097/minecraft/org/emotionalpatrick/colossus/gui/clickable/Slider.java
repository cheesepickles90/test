...
public class Slider {
	...
	public Slider(ValuesBase v, int x, int y, int w, int h, float m, String s, float f, PanelBase d) {
		this.x = x;
		this.y = y;
		width = w;
		height = h;
	...
	public Slider(ValuesBase v, int x, int y, int w, int h, float m, String s, PanelBase d) {
		this.x = x;
		this.y = y;
		width = w;
		height = h;
	...
	public void draw(int xx, int yy, int i, int j)
	{
		...
		ValuesBase v = value;
		GuiUtils.drawWBorderedGradientRect(draggable.x + 9, yy+y + 1,  sliderX + 11, yy+y+height-6,2,0xff,0xFFFF8C00, 0xFFC36900);
	...
	public void drawSliderFill(float x, float y, float f) {
		...
		Wrapper.getMinecraft().renderEngine
				.bindTexture(Wrapper.getMinecraft().renderEngine
