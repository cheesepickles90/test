...
public class Helper {
	...
	public static boolean debugMode = isInDebugMode();
	
	public static void sendPacket(Packet packet) {
		if (!Wrapper.getMinecraft().isSingleplayer()) {
			Wrapper.getMinecraft().getSendQueue().addToSendQueue(packet);
		}
	}
	...
	public static void addChat(String message) {
		if(message != null)
			Wrapper.getPlayer().addChatMessage(ChatColor.GOLD + "[Colossus]: " + ChatColor.WHITE + message);
	}
	...
	public static boolean isFriend (String s) {
		Colossus.getManager();
		NameProtect protect = (NameProtect) ModuleManager.getModuleByName("Name Protect");
		String n = StringUtils.stripControlCodes(s);
		if (NameProtect.getOriginalName().contains(n) || NameProtect.getProtectedName().contains(n)) {
			return true;
		}
		return false;
	}
	...
	public static boolean isEnemy (String s) {
		NameProtect protect = (NameProtect) ModuleManager.getModuleByName("Name Protect");
		String n = StringUtils.stripControlCodes(s);
		if (protect.getEnemies().contains(n)) {
			return true;
		}
		return false;
	}
			...
			for (Object o : Wrapper.getWorld().playerEntities) {
				if (o != null && !(o instanceof EntityPlayerSP)) {
					...
					if (!e.isDead) {
						if (nearest == null) {
							nearest = e;
						} else if (nearest.getDistanceToEntity(Wrapper
								.getPlayer()) > e
								.getDistanceToEntity(Wrapper
										.getPlayer())) {
							nearest = e;
						}
					}
	...
	public static void handleJump() {
		...
		if (ep.isPotionActive(Potion.jump)) {
			ep.motionY += (ep.getActivePotionEffect(
					Potion.jump).getAmplifier() + 1) * 0.1F;
		}
		if (ep.isSprinting()) {
			float var27 = ep.rotationYaw * 0.017453292F;
			ep.motionX -= MathHelper.sin(var27) * 0.2F;
			ep.motionZ += MathHelper.cos(var27) * 0.2F;
		}
		ep.isAirBorne = true;
	}
	...
	public static boolean isInDebugMode() {
		if (getPath().endsWith(".jar")) {
			addConsole("Colossus is running as an obfuscated JAR.");
			return false;
		}
		addConsole("Colossus is running in debug mode.");
		return true;
	}
