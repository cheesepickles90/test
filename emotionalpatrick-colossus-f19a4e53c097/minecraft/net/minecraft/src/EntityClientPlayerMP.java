...
public class EntityClientPlayerMP extends EntityPlayerSP
{
    ...
    {
    	HookManager.onPreMotionUpdate();
        if (this.worldObj.blockExists(MathHelper.floor_double(this.posX), 0, MathHelper.floor_double(this.posZ)))
        {
        	super.onUpdate();
        	if (!Colossus.getManager().getModuleByName("Freecam").isEnabled())
        		this.sendMotionUpdates();
        	return;
        }
        HookManager.onPostMotionUpdate();
    }
    ...
    {
    	Retard retard = (Retard)Colossus.getManager().getModuleByName("retard");
    	HookManager.onTickIngame();
        ...
        // TODO: Colossus
        float fRotationYaw = retard.isEnabled() ? retard.fakeYaw : rotationYaw;
        float fRotationPitch = retard.isEnabled() ? retard.fakePitch : rotationPitch;
        
        if (this.ridingEntity != null)
        {
            this.sendQueue.addToSendQueue(new Packet13PlayerLookMove(this.motionX, -999.0D, -999.0D, this.motionZ, fRotationYaw, fRotationPitch, this.onGround));
            var13 = false;
        }
        else if (var13 && var14)
        {
            this.sendQueue.addToSendQueue(new Packet13PlayerLookMove(this.posX, this.boundingBox.minY, this.posY, this.posZ, fRotationYaw, fRotationPitch, this.onGround));
        }
        else if (var13)
        {
        ...
        {
            this.sendQueue.addToSendQueue(new Packet12PlayerLook(fRotationYaw, fRotationPitch, this.onGround));
        }
        else
        {
    ...
    {
        Colossus.getCommands().runCommands(par1Str);
