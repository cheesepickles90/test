...
import me.ownage.xenon.hacks.classes.AutoTool;
import me.ownage.xenon.hacks.classes.Crits;
...
public class XPlayerController
{
    ...
    protected ItemStack field_85183_f = null;
    ...
    public XPlayerController(Minecraft par1Minecraft, NetClientHandler par2NetClientHandler)
    {
     ...
     */
    public static void clickBlockCreative(Minecraft par0Minecraft, XPlayerController par1PlayerControllerMP, int par2, int par3, int par4, int par5)
    {
    ...
    public void clickBlock(int par1, int par2, int par3, int par4)
    {
    	for(XenonMod mod : Hacks.hackList) {
    		mod.onBlockClicked(par1, par2, par3);
    	}
        if (!this.currentGameType.isAdventure() || this.mc.thePlayer.canCurrentToolHarvestBlock(par1, par2, par3))
        {
            if (this.currentGameType.isCreative())
    ...
    public void onPlayerDamageBlock(int par1, int par2, int par3, int par4)
    {
        ...
        {
            this.blockHitDelay = Hacks.findMod(SpeedMine.class).isEnabled() ? 1 : 5;
            this.netClientHandler.addToSendQueue(new Packet14BlockDig(0, par1, par2, par3, par4));
            clickBlockCreative(this.mc, this, par1, par2, par3, par4);
        }
        else
        {
            if (par1 == this.currentBlockX && par2 == this.currentBlockY && par3 == this.currentblockZ && this.field_85183_f == this.mc.thePlayer.getHeldItem())
            {
                int var5 = this.mc.theWorld.getBlockId(par1, par2, par3);
                ...
                if (this.curBlockDamageMP >= (Hacks.findMod(SpeedMine.class).isEnabled() ? 1.0F - SpeedMine.mineSpeed.getValue() : 1.0F))
                {
                    this.isHittingBlock = false;
    ...
    public XEntityClientPlayerMP func_78754_a(World par1World)
    {
        return new XEntityClientPlayerMP(this.mc, par1World, this.mc.session, this.netClientHandler);
    }
    ...
    {
    	if(Xenon.getHacks().findMod(Crits.class).isEnabled() && par1EntityPlayer instanceof EntityPlayerSP && mc.thePlayer.onGround)
    	{
    		par1EntityPlayer.motionY = 0.41999998688697815D;
			mc.thePlayer.onCriticalHit(par2Entity);
    	}
    	if(Hacks.findMod(AutoTool.class).isEnabled())
    	{
    		((AutoTool)Hacks.findMod(AutoTool.class)).bestSword(par2Entity);
    	}
        this.syncCurrentPlayItem();
