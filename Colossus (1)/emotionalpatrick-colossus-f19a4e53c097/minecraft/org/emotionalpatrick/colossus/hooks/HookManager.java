...
public class HookManager {
	...
	public static final void onPlayerDeath() {
		for (Module m : ModuleManager.getModules()) {
			if (m.isEnabled()) {
				m.onDeath();
			}
	...
	public static final void onMiddleClick() {
		for (Module m : ModuleManager.getModules()) {
			if (m.isEnabled()) {
				m.onMiddleClick();
			}
	...
	public static final float curBlockDamageSet(Block b, int i1, int i2, int i3,
			float i4) {
		float d = b.getPlayerRelativeBlockHardness(Wrapper.getPlayer(),
				Wrapper.getWorld(), i1, i2, i3);
		Colossus.getManager();
		Iterator i = ModuleManager.getModules().iterator();
		Module m;
		do {
	...
	public static final boolean onChatMessage(Packet3Chat c) {
		boolean flag = true;
		Colossus.getManager();
		Iterator i = ModuleManager.getModules().iterator();
		while (i.hasNext()) {
			...
			if (m.isEnabled()) {
				boolean flag1 = m.onRecieveChat(c);
				if (!flag1) {
					flag = false;
				}
			}
