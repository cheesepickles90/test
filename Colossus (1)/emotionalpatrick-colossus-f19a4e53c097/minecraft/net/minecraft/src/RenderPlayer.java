...
public class RenderPlayer extends RenderLiving
{
    ...
    {
    	if (!Colossus.getManager().nameProtect.isEnabled()) {
    		...
    		if (Minecraft.isGuiEnabled()
    			...
    			float f3 = par1EntityPlayer.isSneaking() ? 36000F : 68000F;
    			if (f2 < f3) {
    				String s = par1EntityPlayer.username;	
    				String var3 = StringUtils.stripControlCodes(par1EntityPlayer.username);
    				if (Helper.isFriend(var3) || Helper.isEnemy(var3)) {
    				...
    				if (!par1EntityPlayer.isSneaking()) {
    					...
    					renderLivingLabel(par1EntityPlayer, s, par2, par4, par6,
    							128, 0xffffff);
    				}
    			}
    		}
    ...
    protected void renderLivingLabel(EntityLiving par1EntityLiving, String par2Str, double par3, double par5, double par7, int par9, int j) {
		Wrapper.getRenderer().disableLightmap(0);
    	String s1 = par2Str;
    	int color = 0xFFFFFFFF;
    	float f = par1EntityLiving.getDistanceToEntity(renderManager.livingPlayer);
        FontRenderer var10 = this.getFontRendererFromRenderManager();
        
        float f1 = 1.6F;
        float f2 = 0.016666668F * f1;
        float pX = (float)(par1EntityLiving.posX - this.renderManager.viewerPosX);
        float pY = (float)(par1EntityLiving.posY - this.renderManager.viewerPosY);
        float pZ = (float)(par1EntityLiving.posZ - this.renderManager.viewerPosZ);
        float helpFactor = MathHelper.sqrt_float(pX * pX + pY * pY + pZ * pZ);
        float scaleFactor = helpFactor * 0.00263F + 0.04237F;
        ...
        byte byte0 = (byte) Math.round(scaleFactor * -0.045F - 5.955F);
    	
    	if (Colossus.getManager().nameProtect.isEnabled()) {
    		for (int pCheckLoop = 0; pCheckLoop < Colossus.getManager().nameProtect
    				.getOriginalName().size(); pCheckLoop++) {
    			String pN = ""
    					+ Colossus.getManager().nameProtect.getOriginalName().get(
    							pCheckLoop);
    			if (par2Str.equalsIgnoreCase(pN)) {
    				par2Str = par2Str.replaceAll(pN,
    						""
    								+ Colossus.getManager().nameProtect
    								.getProtectedName().get(pCheckLoop));
    			}
    		}
    	}
    	...
    	if (par1EntityLiving.isSneaking()) {
    		...
    		color = 0xff00ff00;
    	} else if (Helper.isEnemy(par2Str)) {
    		color = 0xAA0000;
    	}
    	...
    	tessellator.setColorRGBA_F(0.0F, 0F, 0.0F, 0.25F);
    	tessellator.addVertex((double)(-i) - 1.5D, -1.5D + (double)byte0, 0.0D);
    	tessellator.addVertex((double)(-i) - 1.5D, 9.5D + (double)byte0, 0.0D);
    	tessellator.addVertex((double)i + 1.5D, 8.5D + (double)byte0, 0.0D);
    	tessellator.addVertex((double)i + 1.5D, -1.5D + (double)byte0, 0.0D);
    	tessellator.draw();
    	GL11.glEnable(GL11.GL_TEXTURE_2D);
    	fontrenderer.drawStringWithShadow(par2Str,
    			-fontrenderer.getStringWidth(par2Str) / 2, byte0, Helper.isFriend(par2Str) ? 0xFFAA00 : color);
    ...
    protected void renderSpecials(EntityPlayer par1EntityPlayer, float par2)
    {
        ...
        float var11;
		boolean load = this.loadDownloadableImageTexture(
				par1EntityPlayer.playerCloakUrl, (String) null);
		boolean shouldload = false;

		shouldload = Helper.isFriend(par1EntityPlayer.username);

		if (load || shouldload)
		{
			if(!load){
				this.loadTexture("/misc/colossus/cape.png");
			}
            GL11.glPushMatrix();
