...
public class RenderLiving extends Render
{
    ...
    public void doRenderLiving(EntityLiving par1EntityLiving, double par2, double par4, double par6, float par8, float par9)
    {
        ...
        /** ESP */
        Tracers tracers = (Tracers)Colossus.getManager().getModuleByName("Tracers");
        if (tracers.isEnabled()) { tracers.renderESP(par2, par4, par6, par1EntityLiving); }
		Wrapper.getRenderer().enableLightmap(0);
    }
    ...
    protected void renderLivingLabel(EntityLiving par1EntityLiving, String par2Str, double par3, double par5, double par7, int par9)
    {
    	if (!Colossus.getManager().nameProtect.isEnabled()) { 
    		...
    		if (par1EntityLiving.isSneaking()) {
    			...
    			color = 0xff00ff00;
    		} else if (Helper.isFriend(par2Str)) {
        		color = 0xFFAA00;
        	}
    		...
    		tessellator.draw();
    		GL11.glEnable(GL11.GL_TEXTURE_2D);
    		for (int pCheckLoop = 0; pCheckLoop < Colossus.getManager().nameProtect.originalNames.size(); pCheckLoop++) {
    			String pN = "" + Colossus.getManager().nameProtect.originalNames.get(pCheckLoop);
    			if (par2Str.equalsIgnoreCase(pN)) {
    				par2Str = par2Str.replaceAll(pN,
    						"" + Colossus.getManager().nameProtect.protectedNames.get(pCheckLoop));
    			}
    		}
    		fontrenderer.drawStringWithShadow(par2Str,
    				-fontrenderer.getStringWidth(par2Str) / 2, byte0, color);
    		GL11.glEnable(GL11.GL_DEPTH_TEST);
    		GL11.glDepthMask(true);
    		for (int pCheckLoop = 0; pCheckLoop < Colossus.getManager().nameProtect.originalNames.size(); pCheckLoop++) {
    			String pN = "" + Colossus.getManager().nameProtect.originalNames.get(pCheckLoop);
